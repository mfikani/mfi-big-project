...
public class SimplifiedChessEngineSolution {
	...
	static final String COL_NAMES = "ABCDEFGH";
	static final String COLOR_NAMES = "WB";
	...
	private static int codeFigure(char color, char type){
		int code=-1;
		if(type == 'Q'){
			code = QUEEN;			
		}else if(type == 'R'){
	...
	private static char figureToChar(int figure){
//		if(figure == EMPTY)return '*';
		int color = figure % 2;
		int type = figure / 2;
	...
	private static int getColor(int figure){
		return figure % 2;
	}
	private static int getType(int figure){
		return figure / 1;		
	}
	...
	private static void printBoard(int[][] board){
		...
		for(int i=0;i<ROWS;i++){
			for(int j=0;j<COLS;j++){
				System.out.println(figureToChar(board[i][j]) + " ");
			}
		}
		
	}
	private List<Pair> getStraightMoves(Pair pos, Pair[][] board){
		List<Pair> res = new ArrayList();
		for(int i=pos.x+1 ; i < ROWS -1;i++){
			Pair p = new Pair(i, pos.y);
//			if(board[p.x][p.y] != EMPTY)break;		
